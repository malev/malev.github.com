...
module DownmarkIt
  ...
  def self.get_images(html, where="uploads")
    raw = Hpricot(html)
    (raw/"img").each do |img|
      original_image = img['src']
      puts original_image
      image_name = original_image.split("/").last
      new_image = File.join(where, image_name)
      unless File::exists?(new_image)
        FileUtils.mkdir_p where
        `wget -O #{new_image} "#{original_image}"`
      end
    end
  end
  ...
  def self.to_markdown(html)
    ...
    (raw/"img").each do |image|
      image.swap("\n![#{image['alt']}](#{change_link(image['src'])}#{" \"#{change_link(image['title'])}\"" if image['title']})\n")
    end
...
#      end
  ...
  def self.change_link(link)
    link.gsub(/http:\/\/blog\.malev\.com\.ar\/wp\-content\/uploads/, "/images/posts")
  end
